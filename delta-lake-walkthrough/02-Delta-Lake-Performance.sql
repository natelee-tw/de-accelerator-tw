...
-- MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&aip=1&t=event&ec=dbdemos&ea=VIEW&dp=%2F_dbdemos%2Fdata-engineering%2Fdelta-lake%2F02-Delta-Lake-Performance&cid=3246944839473348&uid=773523978734231">
...
-- DBTITLE 1,Init the demo data under ${raw_data_location}/user_parquet. Make sure that reset_all_data flag set to "false" below
...
-- MAGIC
-- MAGIC **Delta Liquid Clustering requires DBR 13.3**
-- MAGIC
...
-- DBTITLE 1,Try running a query and note how much time it takes
...
-- MAGIC %md-sandbox
-- MAGIC **So what happened here after trigger Liquid Clustering ?**
-- MAGIC
-- MAGIC You will observe that the output of Cmd 10 above shows a meterics column containing a JSON object with fields `numFilesAdded` and `numFilesRemoved`with values 1 and 2 respectively. This means that the data in `user_clustering` table comprised of 2 files which got compacted into 1 file. 

-- COMMAND ----------

-- DBTITLE 1,Don't forget to the vacuum table periodically to remove your history and previous files
VACUUM user_clustering;

-- COMMAND ----------

-- DBTITLE 1,Our requests using firstname and lastname are now super fast. Almost half of the time taken for same query in Cmd 7 !
SELECT * FROM user_clustering where firstname = 'Teresa'
...
-- let's compact our table. Note that the engine decided to compact 2 files into 1 ("numFilesAdded": 1, "numFilesRemoved": 2)
OPTIMIZE user_delta 
