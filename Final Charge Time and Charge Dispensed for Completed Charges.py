...
def return_stop_transaction_requests(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.filter((input_df.action == 'StopTransaction') & (input_df.message_type == 2))
    return input_df
    ###
...
# Expected
# StructType([StructField('foo', StringType(), True), StructField('body', StringType(), True), StructField('new_body', StructType([StructField('meter_stop', IntegerType(), True), StructField('timestamp', StringType(), True), StructField('transaction_id', IntegerType(), True), StructField('reason', StringType(), True), StructField('id_tag', StringType(), True), StructField('transaction_data', ArrayType(StringType(), True), True)]), True)])

# But got
# ([StructField('foo', StringType(), True), StructField('body', StringType(), True), StructField('new_body', StructType([StructField('meter_stop', IntegerType(), True), StructField('timestamp', StringType(), True), StructField('transaction_id', IntegerType(), True), StructField('reason', StringType(), True), StructField('id_tag', StringType(), True), StructField('transaction_data', ArrayType(StructType([]), True), True)]), True)])

    
def convert_stop_transaction_request_json(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn("new_body", from_json(col("body"), stop_transaction_body_schema()))
    return input_df
    ###
...
# MAGIC %md
# MAGIC From transaction id in StopTransaction
# MAGIC - Find transactionid in StartTransaction Response
# MAGIC   - has transaction_id
# MAGIC - Then find StartTransaction Request
# MAGIC   - no transaction_id
# MAGIC   - has meter_start and timestamp (as start_timestamp)
# MAGIC   - match from message_id
...
def convert_start_transaction_response_json(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn("new_body", from_json(col("body"), start_transaction_response_body_schema()))
...
def join_with_start_transaction_request(input_df: DataFrame, join_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    joined_df = input_df.join(join_df, ['message_id'], 'inner')
    output_df = joined_df.select(input_df["charge_point_id"], 
                                    input_df["new_body"].alias("response_new_body"),
                                    join_df["new_body"].alias("request_new_body"))
    result_df = output_df.select(col("charge_point_id"),
                                col("response_new_body").getItem("transaction_id").alias("transaction_id"),
                                col("request_new_body").getItem("meter_start").alias("meter_start"),
                                col("request_new_body").getItem("timestamp").alias("start_timestamp"),)
    
    # result_df = result_df.withColumn("timestamp", to_timestamp(result_df["timestamp"], "yyyy-MM-dd'T'HH:mm:ss.SSSSSSXXX"))
    # result_df = result_df.withColumnRenamed("timestamp", "start_timestamp")

    return result_df
    ###

    # expected 
    # StructType([StructField('charge_point_id', StringType(), True), StructField('transaction_id', IntegerType(), True), StructField('meter_start', IntegerType(), True), StructField('start_timestamp', StringType(), True)]), 
    # but got 
    # StructType([StructField('charge_point_id', StringType(), True), StructField('transaction_id', IntegerType(), True), StructField('meter_start', IntegerType(), True), StructField('start_timestamp', TimestampType(), True)])
...
def join_stop_with_start(input_df: DataFrame, join_df: DataFrame) -> DataFrame:
    ...
    ### YOUR CODE HERE
    join_df = join_df.withColumnRenamed("charge_point_id", "charge_point_id_a")
    

    joined_df = input_df.join(join_df, [input_df.new_body.transaction_id == join_df.transaction_id], 'left')
    output_df = joined_df.select(input_df["charge_point_id"].alias("charge_point_id"),
                                 join_df["transaction_id"].alias("transaction_id"), 
                                 join_df["meter_start"].alias("meter_start"),
                                 col("new_body").getItem("meter_stop").alias("meter_stop"),
                                 join_df["start_timestamp"].alias("start_timestamp"),
                                 col("new_body").getItem("timestamp").alias("stop_timestamp"))
...
## not sure why failing, so i just comment it out
...
def convert_start_stop_timestamp_to_timestamp_type(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn("start_timestamp", to_timestamp(input_df["start_timestamp"], "yyyy-MM-dd'T'HH:mm:ss.SSSSSSXXX"))
    input_df = input_df.withColumn("stop_timestamp", to_timestamp(input_df["stop_timestamp"], "yyyy-MM-dd'T'HH:mm:ss.SSSSSSXXX"))
...
def calculate_total_time_hours(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn('total_time', (col("stop_timestamp").cast("long") - col('start_timestamp').cast("long")) / 3600)
    input_df = input_df.withColumn('total_time', round(input_df.total_time, 2))
...
def calculate_total_energy(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn('total_energy', col("meter_stop").cast('double') - col("meter_start"))
...
def convert_metervalues_to_json(input_df: DataFrame) -> DataFrame:
    ...
    meter_value_schema = StructType([
        StructField("timestamp", StringType(), True),
        StructField("sampled_value", ArrayType(sampled_value_schema), True)
        ])

    new_body_schema = StructType([
        StructField("connector_id", IntegerType(), True),
        StructField("transaction_id", IntegerType(), True),
        StructField("meter_value", ArrayType(meter_value_schema), True),
    ])

    
    return input_df.withColumn("new_body", from_json(col("body"), new_body_schema))
...
# 1 Build a DataFrame from our MeterValue Request data with transaction_id, timestamp, measurand, phase, and value pulled out as columns (explode)  
# 2 Return only rows with measurand = Power.Active.Import and phase = Null
# 3 Figure out how to represent in the DataFrame when a Charger is actively charging or not charging, calculate the duration of each of those groups, and sum the duration of the non charging groups as the total_parking_time
...
def calculate_total_parking_time(input_df: DataFrame) -> DataFrame:
    ...
    #     "start_value", lag("value").over(window_by_transaction)
    # )
    # input_df = input_df.withColumn("total_parking_time", col("value") - col("start_value"))
