...
def create_dataframe(filepath: str) -> DataFrame:
    ...
    df = spark.read.csv(filepath, schema=custom_schema, header=True)
    return df
    ###
...
df.select("write_timestamp").show(1, truncate=False)
...
def convert_to_timestamp(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn("converted_timestamp", to_timestamp(input_df["write_timestamp"], "yyyy-MM-dd'T'HH:mm:ss.SSSSSSXXX"))

    return input_df
    ###
# 2023-01-01T08:01:29.000013+00:00 
...
def most_recent_message_of_charge_point(input_df: DataFrame) -> DataFrame:
    ranking_window = Window.partitionBy('charge_point_id').orderBy(desc('converted_timestamp'))


    ### YOUR CODE HERE
    input_df = input_df.withColumn('rn', row_number().over(ranking_window)
                    )

    input_df = input_df.filter(input_df.rn == 1)
...
def cleanup(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.drop(input_df.rn)
    ###
